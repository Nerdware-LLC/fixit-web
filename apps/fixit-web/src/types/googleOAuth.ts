...
export interface IdConfiguration {
  ...
  itp_support?: boolean;
  /**
   * If your application knows the Workspace domain the user belongs to, use this to provide a hint
   * to Google. For more information, see the [hd][google-oidc-hd] field in the OpenID Connect docs.
   *
   * [google-oidc-hd]: https://developers.google.com/identity/protocols/oauth2/openid-connect#authenticationuriparameters
   */
...
export type NonOAuthError = {
  /**
   * Some non-OAuth errors, such as the popup window failed to open, or closed
   * before an OAuth response was returned.
   *
...
export interface TokenClientConfig {
   ...
   * popup window failed to open, or closed before an OAuth response was returned.
   */
  ...
  /**
   * Optional, defaults to true. If set to false, [more granular Google Account permissions][link]
   * will be disabled for clients created before 2019. No effect for newer clients, since more
   * granular permissions is always enabled for them.
   *
   * [link]: https://developers.googleblog.com/2018/10/more-granular-google-account.html
   */
  enable_serial_consent?: boolean;

  /**
   * Optional. If your app knows which user should authorize the request, it can use this property
   * to provide a hint to Google. The email address for the target user. For more information, see
   * the [login_hint](https://developers.google.com/identity/protocols/oauth2/openid-connect#authenticationuriparameters) field in the OpenID Connect docs.
   */
  hint?: string;

  /**
   * Optional. If your application knows the Workspace domain the user belongs to, use this to
   * provide a hint to Google. For more information, see the [hd][hd-link] field in the OIDC docs.
   *
   * [hd-link]: https://developers.google.com/identity/protocols/oauth2/openid-connect#authenticationuriparameters
   */
  hosted_domain?: string;

  /**
   * Optional. Not recommended. Specifies any string value that your application uses to
   * maintain state between your authorization request and the authorization server's response.
   */
  state?: string;
}
...
export interface OverridableTokenClientConfig {
  ...
  /**
   * Optional. If set to false, [more granular Google Account permissions][link] will be disabled
   * for clients created before 2019. No effect for newer clients, since more granular permissions
  ...
  /**
   * Optional. If your application knows which user should authorize the request, it can use this
   * property to provide a hint to Google. The email address for the target user. For more info, see
   * the [login_hint](https://developers.google.com/identity/protocols/oauth2/openid-connect#authenticationuriparameters) field in the OpenID Connect docs.
   */
  hint?: string;

  /**
   * Optional. Not recommended. Specifies any string value that your application uses to maintain
...
export interface CodeClientConfig {
  /**
   * Required. The client ID for your application. You can find this value in the [API Console](https://console.developers.google.com/)
   */
  client_id: string;

  /**
   * Required. A space-delimited list of scopes that identify the resources that your application
   * could access on the user's behalf. These values inform the consent screen that Google displays
   * to the user
   */
  scope: string;
  ...
  /**
   * Required for redirect UX. Determines where the API server redirects the user after the user
   * completes the authorization flow. The value must exactly match one of the authorized redirect
   * URIs for the OAuth 2.0 client, which you configured in the API Console and must conform to our
  ...
  /**
   * Optional. Recommended for redirect UX. Specifies any string value that your application uses
   * to maintain state between your authorization request and the authorization server's response
   */
  state?: string;
...
export interface GoogleOAuthClient {
  accounts: {
    id: {
      ...
      cancel: () => void;
      onGoogleLibraryLoad: (...args: unknown[]) => unknown;
      revoke: (accessToken: string, done: () => void) => void;
    };

    oauth2: {
      initTokenClient: (config: TokenClientConfig) => GoogleOAuthTokenClient;
      initCodeClient: (config: CodeClientConfig) => GoogleOAuthCodeClient;
      hasGrantedAnyScope: (
